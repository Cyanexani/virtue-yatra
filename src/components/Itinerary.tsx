...
import { MapPin, Lightbulb, Loader2, Sparkles, HelpCircle, Download, Share2, RefreshCw } from "lucide-react";
...
const Itinerary = ({ destination, startDate, endDate, interests, travelers, budget, specialRequests }: ItineraryProps) => {
  ...
  // FIX: fetchKey lets the user manually re-trigger the AI call via a button
  ...
  // FIX: wrap in useCallback so it can be called manually via button too
  const fetchAI = useCallback(async () => {
    ...
    // FIX: append T00:00:00 to avoid UTC off-by-one on date parsing
    ...
    try {
      ...
      if (!cancelled) setAiError(e instanceof Error ? e.message : "AI Planner unavailable");
        ...
        mockItinerary.push({
          ...
          activities: [
            ...
            },
            {
              ...
              description: i === 0 ? `A secluded gem completely away from the tourist crowds.` : i === 1 ? `Escape the city noise in these beautiful, lush green pathways.` : `Watch local craftsmen at work and pick up authentic souvenirs.`
            },
            {
              time: "Evening",
              spot: i === 0 ? `${destination} Sunset Point & Cafe` : i === 1 ? `Riverside Dining at ${destination}` : `Vibrant Night Market`,
              type: i === 2 ? "Shopping" : "Leisure",
              description: i === 0 ? `Unwind at this highly rated cafe while watching the sunset.` : i === 1 ? `A premium culinary experience featuring the best local delicacies.` : `Immerse yourself in the bustling night energy and street food.`
            }
          ]
        });
      }
      ...
      const mockResponse: AIResponse = {
        ...
        food_recommendations: { local_specialties: ["Local Street Food Platter", "Traditional Thali", "Spiced Tea/Coffee"] },
        safety_guide: {
          risk_level: "Low",
  ...
  // FIX: fetchKey in deps so the "Regenerate" button re-triggers the effect
  useEffect(() => {
    if (destination && startDate && endDate) {
      const cancel = fetchAI();
  ...
  return (
    <Card className="p-6 mt-8 border-border/50 bg-card/80 backdrop-blur-sm animate-slide-up">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {aiLoading && (
            <span className="flex items-center gap-2 text-xs text-muted-foreground bg-muted/50 px-3 py-1.5 rounded-full">
              <Loader2 className="w-4 h-4 text-primary animate-spin" /> Google Gemini Generating Plan...
            </span>
          )}
          {aiData?.itinerary && !aiLoading && !aiData.isMock && (
            <span className="flex items-center gap-1.5 text-xs text-primary font-medium bg-primary/10 px-3 py-1.5 rounded-full border border-primary/20 shadow-sm">
              <Sparkles className="w-4 h-4" /> Generated by Google Gemini AI
            </span>
          )}
          {aiData?.itinerary && !aiLoading && aiData.isMock && (
            <span className="flex items-center gap-1.5 text-xs text-amber-600 font-medium bg-amber-500/10 px-3 py-1.5 rounded-full border border-amber-500/20 shadow-sm">
              <Sparkles className="w-4 h-4" /> Sample Plan (AI unavailable)
            </span>
          )}
          {/* FIX: Regenerate button so users can retry without re-submitting the whole form */}
          {!aiLoading && (
            <Button
              variant="outline"
              size="sm"
      ...
      {aiData?.isMock && (
        <div className="p-3 bg-amber-500/10 text-amber-600 rounded-lg mb-4 text-sm">
          The AI planner couldn't be reached, so this is a sample itinerary{aiError ? ` (${aiError})` : ""}. Hit Regenerate to try again.
        </div>
      )}

      {aiData?.error && (
        <div className="p-4 bg-amber-500/10 text-amber-500 rounded-lg mb-4 text-sm font-semibold">
          AI Planner Error: {aiData.error}
        </div>
      )}
        ...
        <div ref={itineraryRef} className="p-2 bg-background rounded-lg">
          <div className="flex justify-between items-center mb-6 bg-muted/30 p-4 rounded-lg flex-wrap gap-4">
            <div>
              ...
              <p className="font-bold text-lg">₹{aiData.total_cost?.toLocaleString('en-IN')}</p>
            </div>
            <div>
              ...
              {(provided) => (
                <div {...provided.droppableProps} ref={provided.innerRef} className="space-y-6 mt-6">
                  {localItinerary.map((dayItem, idx) => (
                    // FIX: use stable uuid id as key, not destination+idx which collides on same-city multi-day
                    <Draggable key={dayItem.id} draggableId={dayItem.id!} index={idx}>
                      {(provided) => (
                        <div
                        ...
                        >
                          ...
                          <div className="flex gap-6 text-sm text-muted-foreground bg-muted/20 p-4 rounded-lg border border-border/30 mb-4">
                            <p><strong>Cost Constraint:</strong> ₹{dayItem.cost.toLocaleString('en-IN')}</p>
            ...
            <div className="grid md:grid-cols-2 gap-6 mt-8 animate-slide-up">
              <Card className="p-5 bg-card/80 border-border/50 shadow-sm hover:shadow-md transition-all">
                ...
                <div className="space-y-3 text-sm">
                  <div className="flex justify-between items-center p-2 rounded-lg bg-muted/30"><span>🏨 Accommodation</span><span className="font-semibold">₹{aiData.budget_breakdown.accommodation.toLocaleString('en-IN')}</span></div>
                  <div className="flex justify-between items-center p-2 rounded-lg bg-muted/30"><span>🍽️ Food</span><span className="font-semibold">₹{aiData.budget_breakdown.food.toLocaleString('en-IN')}</span></div>
                  <div className="flex justify-between items-center p-2 rounded-lg bg-muted/30"><span>🚇 Transport</span><span className="font-semibold">₹{aiData.budget_breakdown.transportation.toLocaleString('en-IN')}</span></div>
                  <div className="flex justify-between items-center p-2 rounded-lg bg-muted/30"><span>🎟️ Activities</span><span className="font-semibold">₹{aiData.budget_breakdown.activities.toLocaleString('en-IN')}</span></div>
                  <div className="flex justify-between items-center p-2 rounded-lg bg-muted/30"><span>🛍️ Shopping</span><span className="font-semibold">₹{aiData.budget_breakdown.shopping.toLocaleString('en-IN')}</span></div>
                  <div className="h-px bg-border/50 my-3" />
                  <div className="flex justify-between items-center p-2 rounded-lg bg-primary/10 text-primary font-bold text-base"><span>Total Estimated</span><span>₹{aiData.budget_breakdown.total_estimated.toLocaleString('en-IN')}</span></div>
                </div>
              </Card>
                  ...
                  <div className="space-y-4">
                    {aiData.accommodations.map((acc, i) => (
                      <div key={i} className="text-sm border-l-2 border-primary/50 pl-4 py-1">
                        ...
                        <p className="text-primary font-semibold mt-1">Est. ₹{acc.estimated_cost.toLocaleString('en-IN')}/night</p>
                      </div>
                    ))}
                  </div>
                </Card>
                ...
                <div className="text-sm">
                  ...
                  <ul className="space-y-2 pl-2">
                    {aiData.food_recommendations.local_specialties.map((f, i) => (
                      <li key={i} className="flex items-center gap-2 text-muted-foreground"><span className="w-1.5 h-1.5 rounded-full bg-primary/50" />{f}</li>
                    ))}
                  </ul>
                </div>
              </Card>
                ...
                <div className="text-sm">
                  ...
                  <ul className="space-y-2 pl-2 mb-3">
                    {aiData.safety_guide.common_scams.map((f, i) => (
                      <li key={i} className="flex items-start gap-2 text-muted-foreground"><span className="w-1.5 h-1.5 rounded-full bg-amber-500/50 mt-1.5 flex-shrink-0" /><span>{f}</span></li>
                    ))}
                  </ul>
                  {aiData.safety_guide.emergency_info && (
                    <p className="text-xs bg-destructive/5 text-destructive p-2 rounded font-medium">🚨 {aiData.safety_guide.emergency_info}</p>
                  )}
                </div>
              </Card>
            )}
