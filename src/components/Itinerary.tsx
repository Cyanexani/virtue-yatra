    ...
    const fetchAI = async () => {
      ...
      try {
        ...
        if (isGeminiConfigured()) {
          ...
          const { data: edgeData, error: edgeError } = await supabase.functions.invoke('generate-itinerary', {
            body: { 
              destination, 
              budget: numericBudget, 
              startDate,
    ...
    <Card className="p-6 mt-8 border-border/50 bg-card/80 backdrop-blur-sm animate-slide-up">
      <div className="flex items-center justify-between gap-3 mb-6 flex-wrap">
        ...
        {aiData?.itinerary && !aiLoading && (
          <span className={`flex items-center gap-1.5 text-xs font-medium px-3 py-1.5 rounded-full border shadow-sm ${aiData.isMock ? 'text-amber-600 bg-amber-500/10 border-amber-500/20' : 'text-primary bg-primary/10 border-primary/20'}`}>
            <Sparkles className="w-4 h-4" /> {aiData.isMock ? "Sample Plan (AI unavailable)" : isGeminiConfigured() ? "Generated by Google Gemini AI" : "AI Agent Generated"}
          </span>
        )}
      </div>
      
      {/* Show the aiError prominently if we are using the local mock fallback */}
      {aiData?.error && aiData?.isMock && (
        <div className="p-4 bg-amber-500/10 text-amber-600 rounded-lg mb-4 text-sm font-semibold flex flex-col gap-1">
          <span>AI Planner Fallback: Showing local sample.</span>
          <span className="text-xs opacity-80">Reason: {aiData.error}</span>
        </div>
      )}
