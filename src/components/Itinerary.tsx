  ...
  useEffect(() => {
    ...
    const fetchAI = async () => {
      ...
      try {
        ...
        // Primary path: Supabase edge function (Gemini via Lovable AI gateway)
        ...
        // Client-side Gemini fallback, if a key is configured locally
    ...
    <Card className="p-6 mt-8 border-border/50 bg-card/80 backdrop-blur-sm animate-slide-up">
      ...
      {aiData?.isMock && (
        <div className="p-3 bg-amber-500/10 text-amber-600 rounded-lg mb-4 text-sm">
          The AI planner couldn't be reached, so this is a sample itinerary{aiError ? ` (${aiError})` : ""}. Try regenerating in a moment.
        </div>
      )}
